...
/* ===============================
   🛠️ SPA SAFETY FIX — RESET STYLES
   Prevent disappearing GIF / black gaps
   =============================== */
...
(() => {
  ...
  function revealVideoCTA() {
    const cta = document.querySelector('.edits-cta');
    if (!cta) return;
  ...
  // initial load
  document.addEventListener('DOMContentLoaded', revealVideoCTA);

  // expose for SPA re-use
  window.revealVideoCTA = revealVideoCTA;
